...
const int Num_LOAD_RS = 2; // Load Op Functionality
...
const int MultOp = 2;
const int DivOp = 3;
const int LoadOp = 4; // Load Op Functionality
...
// Modified latencies to match that of the examples given to have a more meaningful conclusion and comparison later
const int LOAD_Lat = 2; // Load Op Functionality
const int ADD_Lat = 2;
...
int main()
{
    ...
    vector<ReservationStation> ResStation = {LOAD1,
                                             LOAD2,
                                             ADD1,
                                             ADD2,
                                             ADD3,
          ...
          vector<int> &REG)
{
    ...
    int RSLoadStart = Num_ADD_RS + Num_MULT_RS + Num_DIV_RS;
    int RSLoadEnd = Num_ADD_RS + Num_MULT_RS + Num_DIV_RS + Num_LOAD_RS;
    switch (r)
    {
    case LoadOp:
...
void printReservationStations(vector<ReservationStation> RSV)
{
    for (int i = 0; i < RSV.size(); i++)
        cout << "RS #: " << i << "  Busy: " << RSV[i].busy << "  op: " << RSV[i].op << "  Vj: " << RSV[i].Vj << "  Vk: " << RSV[i].Vk << "  Qj: " << RSV[i].Qj << "  Qk: " << RSV[i].Qk << endl;
}
...
void printInstructions(vector<Instruction> IV)
{
    for (int i = 0; i < IV.size(); i++)
        cout << "Instruction #: " << i << "  Operation: " << IV[i].op << "  " << IV[i].rd << " <- " << IV[i].rs << " op " << IV[i].rt << endl;
}
